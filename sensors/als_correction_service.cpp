...
class AlsCorrection {
  public:
    ScreenShotInfo takeScreenshot() {
        ...
        captureArgs.sourceCrop = (state.orientation == Rotation::Rotation0 ||
                                  state.orientation == Rotation::Rotation180)
                                         ? screenshot_rect
                                         : screenshot_rect_land;
...
void AlsCorrection::init() {
    ...
    const std::shared_ptr<ChenALSHelperCallback> mChenALSCallback =
            ndk::SharedRefBase::make<ChenALSHelperCallback>(this);
