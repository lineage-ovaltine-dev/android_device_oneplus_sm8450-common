...
void AlsCorrection::init() {
    ...
    mChenALSHelper = IALSHelper::fromBinder(
    ...
    ALOGI("Screen on time: %.2fh (aging factor: %.2f%%)", screen_on_time,
          screen_aging_factor * 100.0);
...
void AlsCorrection::process(Event& event) {
    ...
    */
    ...
    if (conf.rgbw_max_lux[0] == 0.0 && conf.rgbw_max_lux[1] == 0.0 && conf.rgbw_max_lux[2] == 0.0 &&
        conf.rgbw_max_lux[3] == 0.0) {
        ALOGE("Art_Chen: Maybe EngSensor not finished when init sensorservice, Trying to init "
              "AlsCorrection again! This event will be dropped!");
        AlsCorrection::init();
    ...
    if (state.last_update == 0) {
        ...
        if (brightness > 0.0 &&
            ((now - state.last_forced_update) > s2ns(1) || isRawValueRapidGrowth)) {
    ...
    if (state.force_update ||
        ((event.u.scalar < state.hyst_min || event.u.scalar > state.hyst_max) &&
         (event.u.data[6] > 2.0 || sensor_raw_calibrated < 10.0 ||
          sensor_raw_calibrated > (5.0 / .07)))) {
        // android::base::unique_fd fd(socket(PF_UNIX, SOCK_STREAM | SOCK_CLOEXEC | SOCK_NONBLOCK,
        // 0)); if (fd.get() < 0) {
        ...
        float rgbw[4] = {static_cast<float>(screenshot.r), static_cast<float>(screenshot.g),
                         static_cast<float>(screenshot.b),
        ...
        if (cumulative_correction <= event.u.scalar * 1.35 ||
            event.u.scalar * conf.calib_gain * agc_gain < 10000.0 || state.force_update) {
